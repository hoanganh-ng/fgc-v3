...
- Profile home-feed scheduled dispatch: `DispatchNextDueProfileHomeFeedCollectionScheduleUseCase` selects one eligible due profile-home-feed schedule, performs profile reference lookup outside any database transaction, and persists exactly one queued `SCHEDULED` `ProfileHomeFeedCollectionRun` or a safe skip/defer outcome through a schedule compare-and-set repository. It is not wired to a scheduler poller, worker, browser executor, Docker service, HTTP execution route, or Web UI.
