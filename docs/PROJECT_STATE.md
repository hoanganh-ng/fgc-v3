...
The current focus is the Content Collector stage. Collector Profile Manager is complete through Sprint 013 and accepted as the first backend module. Content Manager backend is complete through Sprint 019. Web UI foundation work started in Sprint 025, read-only Profile Manager list/detail integration was added in Sprint 026, Dockerized full-stack runtime support was added in Sprint 027, structured profile create/configure forms were added in Sprint 028, Start Provisioning UI was added in Sprint 029, the Profile Provisioning Browser CLI was added in Sprint 030, provisioning E2E verification and hardening was completed in Sprint 031, the Facebook Browser Payload Capture Adapter was added in Sprint 032, Web UI content category/source group management was added in Sprint 033, Collector CLI source group resolution and checkout diagnostics were added in Sprint 034A, page-context Facebook fetch/XHR capture was added in Sprint 034B, Content Items review UI screens were added in Sprint 035, Content Items review layout polish was completed in Sprint 035A, collection run records plus API trigger work was completed in Sprint 036, the Collector Worker Process was added in Sprint 037, the Collector Runtime browser-provider boundary plus optional experimental CloakBrowser adapter were added in Sprint 037A, runtime command hygiene plus the root README were completed in Sprint 037A.1, the containerized collector worker runtime was completed in Sprint 037B, the Profile Account Stage Readiness Gate was completed in Sprint 038, the Ambient Account Exercise Foundation was completed in Sprint 039, Source Group Entry Routes Foundation was completed in Sprint 040, Profile-Source Access Domain + Storage was completed in Sprint 041A, Profile-Source Access HTTP API was completed in Sprint 041B, Profile-Source Access Web UI was completed in Sprint 041C, Source-Aware Collection Checkout Gate was completed in Sprint 042, Assisted Group Access Checkout Foundation was completed in Sprint 043, the Operator-Assisted Group Access Browser CLI was completed in Sprint 043A, Operator-Reported Profile-Source Access Outcome was completed in Sprint 043B, Sprint 044 is complete, the Collection Runs Web UI was completed in Sprint 045, the Ambient Exercise Runs Web UI was completed in Sprint 046, the Queued Ambient Exercise Worker was completed in Sprint 047, the Containerized Account Exercise Worker Runtime was completed in Sprint 047A, Category Browse Exercise Foundation was completed in Sprint 048, Category Browse Request Web UI was completed in Sprint 049, the Profile-Source Access Check Run Foundation was completed in Sprint 050, Profile-Source Access Check Run Contract Hardening was completed in Sprint 050A, the Queued Profile-Source Access Check Worker was completed in Sprint 051, the Profile-Source Access Check Runs Web UI was completed in Sprint 052, Facebook Authentication Wall Detection Hardening was completed in Sprint 053, CloakBrowser Provisioning Support was completed in Sprint 053A, Facebook Checkpoint False-Positive Hardening was completed in Sprint 053B, and Sprint 054A introduces the Profile Authentication Health Foundation domain model, persistence, and safe DTO exposure.

## Current Sprint

Sprint 054A is active: Profile Authentication Health Foundation.
Sprint 053 (Facebook Authentication Wall Detection Hardening), Sprint 053A
(CloakBrowser Provisioning Support), and Sprint 053B (Facebook Checkpoint
False-Positive Hardening) are complete after live validation.
...
- Sprint 041C is complete. Profile-source access records can be created and managed through the Web UI profile detail page.
- Sprint 042 introduces source-aware collection checkout. Normal collection checkout requires a source group and leases only profiles with PUBLIC_ACCESSIBLE or JOINED_ACCESSIBLE access state for the requested source.
- Sprint 042 is complete. Normal collection checkout validates source groups before transaction entry, enforces profile-source access, and leaves source-access records unchanged.
- Sprint 043 introduces assisted group access checkout as a distinct Profile Manager lease purpose. Assisted checkout requires an explicit profile and source group, validates source group existence, allows `WARMING` and `COLLECTION_READY` account stages, does not require successful profile-source access, and does not mutate profile-source access records.
- Sprint 043 is complete. Assisted group access checkout and release/runtime-configuration flows are available for future operator-assisted workflows.
- Sprint 043A introduces the operator-assisted group access browser CLI that consumes assisted leases, resolves safe source-group entry routes, launches a headed browser for manual inspection, and releases the lease without automating group actions or mutating access state.
- Future account-readiness roadmap items include Category Browse Exercise, Scheduler behavior, browser-backed access checks, and automatic account-stage recommendations; those remain out of scope for Sprint 043.
...
- Sprint 054A introduces a `ProfileAuthenticationHealth` enum (`NOT_PROVISIONED`, `HEALTHY`, `REAUTH_REQUIRED`, `CHECKPOINT_REVIEW_REQUIRED`) as a separate dimension on `CollectorProfile`, independent of `status` and `accountStage`.
- Sprint 054A adds `authenticationHealth` and `authenticationHealthUpdatedAt` to the persisted profile. New profiles default to `NOT_PROVISIONED` with `authenticationHealthUpdatedAt = createdAt`. Only successful browser authentication-state ingestion (`IngestProfileSessionUseCase`) transitions health to `HEALTHY`, atomic with existing session storage and token invalidation. Starting provisioning, issuing a token, failing provisioning, and canceling provisioning do not change health.
- Sprint 054A backfills existing `READY`/`BUSY` rows to `HEALTHY` and `PENDING_CONFIG`/`PENDING_LOGIN` rows to `NOT_PROVISIONED`, using `updatedAt` (falling back to `createdAt`) as the backfill timestamp.
- Sprint 054A exposes `authenticationHealth` and `authenticationHealthUpdatedAt` in safe profile list and detail responses. Runtime enforcement of health during checkout belongs to Sprint 054B.
