...
Active sprint file: `docs/SPRINTS/SPRINT-043-assisted-group-access-checkout-foundation.md`

Sprint 043 adds the `ASSISTED_GROUP_ACCESS` lease purpose and a specified-profile checkout contract for future operator-assisted source group access sessions. It validates source group existence before opening the Profile Manager transaction, allows `WARMING` and `COLLECTION_READY` profiles that pass the ordinary safety gates, rejects other account stages with a purpose-specific reason, creates an assisted lease, and leaves profile-source access state unchanged.
