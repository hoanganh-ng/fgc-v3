...
and must equal `collectionSurface.sourceGroupId` (and the same
equality rule applies on the durable schema between
`firstCollectionSurface.sourceGroupId` and `managedSourceGroupId`),
permits an absent or present `managedSourceGroupId` for a
`PROFILE_HOME_FEED` input and durable provenance, runtime-validates
its inputs and outputs against the existing Zod domain schemas,
never mutates inputs, and throws a typed
