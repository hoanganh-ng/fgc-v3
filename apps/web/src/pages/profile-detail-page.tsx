...
import { resolveProvisioningAction } from "@/features/profiles/profile-provisioning-action";
...
function StartProvisioningCard({
  ...
  function start(): void {
    ...
    // Capture the success context from the action the operator just
    // submitted. The profile status flips to PENDING_LOGIN after the
    // mutation refetch, so the live `action` would resolve to the
    // PENDING_LOGIN "New provisioning token issued." title. Storing
    // the submitted title in component state preserves it across the
    // refetch without remounting the card.
    const capturedSuccessTitle = action.successTitle;
    ...
    void (async () => {
      try {
        ...
        setProvisioningSuccess({
          successTitle: capturedSuccessTitle,
        ...
        {provisioningSuccess !== null ? (
          ...
          >
            <div className="flex gap-2">
              ...
              />
              <div className="min-w-0">
                <p className="font-semibold">{provisioningSuccess.successTitle}</p>
